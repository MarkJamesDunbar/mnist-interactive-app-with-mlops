...
# Filepath Config
data_folder="../data/"
artefacts_folder="../model_evaluation/"
...
train_set   = mu.load_dataset(train_file, data_folder)
test_set = mu.load_dataset(test_file, data_folder)

# Split the data into train/test splits - split_size defined in config
train_images, test_images, train_labels, test_labels = train_test_split(train_set.iloc[:, 1:], 
                                                                     train_set.iloc[:, 0], 
...
for epoch in range(epochs):
    ...
    lr, trl, tra = mu.train_model(net, device, optimizer, scheduler, training_dataloader, batch_size)
...
with open(artefacts_folder + "metrics.txt", "w") as outfile:
    outfile.write(f"\nAccuracy: {round(accuracy, 4)}\nPrecision: {round(precision, 4)}\nRecall: {round(recall, 4)}\nF1 Score: {round(f1, 4)}")

# Save confusion matrix plot
mu.confusion_matrix(num_classes, test_labels, predictions, save_path=artefacts_folder+"confusion_matrix.png")
...
plt.savefig(artefacts_folder+"nn_lr_curve.png")
...
plt.xticks(epoch_num)
plt.savefig(artefacts_folder+"nn_loss_curve.png")
...
plt.xticks(epoch_num)
plt.savefig(artefacts_folder+"nn_acc_curve.png")

#################################################################################################
# Export Model
#################################################################################################

torch.save(net, "../app/model/model.pth")
